...
export class MapRenderer {
  ...
  constructor(gameMap: GameMap) {
    ...
    this.cellHashMap = this.gameConverter.convertCellsArrayToMap(gameMap.cells);
  ...
  public selectMapCell(canvasContext: any, map: GameMap, x: number, y: number) {
    ...
    map.selected = this.mapProcessor.getCellFromMapByCoordinates(x_cell, y_cell, this.cellHashMap)
  ...
  private clearCurrentSelectedCell(map: GameMap, ctx: any) {
    ...
    ctx.fillStyle = this.mapProcessor.getCellFromMapByCoordinates(selectedX, selectedY, this.cellHashMap).color;
